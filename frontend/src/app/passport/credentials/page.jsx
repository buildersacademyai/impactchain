...
function Nav({active}){
  ...
  const links=[["Passports","/passport/register"],["Disburse","/disburse"],["Oracle","/oracle"],["Transparency","/transparency"],["Dashboard","/agency/dashboard"]];
  return(
    <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:100,padding:"0 5%",height:62,display:"flex",alignItems:"center",justifyContent:"space-between",background:sc?"rgba(7,3,15,.92)":"transparent",backdropFilter:sc?"blur(20px)":"none",borderBottom:sc?"1px solid rgba(255,255,255,.07)":"none",transition:"all .3s"}}>
      <a href="/" style={{fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:18,color:"#f5f3ff",textDecoration:"none",letterSpacing:"-.02em"}}>Impact<span style={{color:G}}>Chain</span></a>
      <div style={{display:"flex",gap:6,alignItems:"center"}}>
        {links.map(([l,h])=>(
          <a key={l} href={h} style={{padding:"6px 14px",borderRadius:10,fontSize:13,fontWeight:600,color:active===l?G:"#94a3b8",background:active===l?"rgba(167,139,250,.1)":"transparent",textDecoration:"none",transition:"all .2s"}}>{l}</a>
        ))}
      </div>
    </nav>
  );
...
export default function CredentialsPage(){
  ...
  async function revoke(index){
    ...
    try{
      ...
      setRevokeOk(d);
      // Optimistically mark revoked in local state
      setPassport(prev=>({
        ...prev,
  ...
  const active = creds.filter(c=>!c.revoked && credStatus(c)==="active").length;
      ...
      <div style={{maxWidth:720,margin:"0 auto",position:"relative",zIndex:2}}>
        ...
        <div className="card" style={{marginBottom:24}}>
          ...
          <div style={{marginBottom:16}}>
            ...
            <input className="ic-input" placeholder="did:ethr:celo:0x..." value={did} onChange={e=>setDid(e.target.value)}
              onKeyDown={e=>e.key==="Enter"&&lookup()} />
          </div>
          <button className="btn-p" onClick={lookup} disabled={loading||!did.trim()||!apiKey.trim()}
          ...
          <div style={{animation:"fadeUp .4s ease"}}>
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:12}}>
                <div>
                  <div style={{fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:16,color:"#f5f3ff",marginBottom:4}}>
                    {passport.name||"Unnamed Beneficiary"}
                  </div>
                  <div className="mono">{passport.did}</div>
                </div>
                <div style={{display:"flex",gap:8,alignItems:"center"}}>
                  <span style={{fontSize:13,color:"#64748b"}}>{active} active credential{active!==1?"s":""}</span>
                  <span className={active>0?"badge-active":"badge-revoked"}>
                    {creds.length} total
                  </span>
                </div>
              </div>
            ...
            {revokeOk&&(
              <div style={{marginBottom:16,padding:"12px 16px",borderRadius:12,background:"rgba(52,211,153,.07)",border:"1px solid rgba(52,211,153,.2)",color:"#34d399",fontSize:13}}>
                ...
                <a href={`https://alfajores.celoscan.io/tx/${revokeOk.tx_hash}`} target="_blank" rel="noreferrer"
                  ...
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:12,flexWrap:"wrap"}}>
                    <div style={{flex:1}}>
                      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8,flexWrap:"wrap"}}>
                        <span style={{fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:14,color:"#f5f3ff"}}>
                          {c.credential_type}
                        </span>
                        <span className={status==="active"?"badge-active":status==="expired"?"badge-expired":"badge-revoked"}>
                          {status}
                        </span>
                    ...
                    {!c.revoked&&(
                      <button className="btn-danger" onClick={()=>revoke(i)} disabled={revoking===i}
            ...
            <div style={{marginTop:20,display:"flex",gap:10}}>
              <button className="btn-g" onClick={()=>{setPassport(null);setDid("");setRevokeOk(null);setRevokeErr("");}}>
                ← New Lookup
              </button>
              <button className="btn-g" onClick={lookup} disabled={loading}>
